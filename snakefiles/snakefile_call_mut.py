...
rule all:
    input: 
        expand(f"DARLIN/{{template}}_{python_sub_dir}/{{sample}}/final.done",sample=SampleList,template=template_list)

rule gen_slim_fastq:
    input:
        called_bc_file=f"DARLIN/{{template}}_{python_sub_dir}/{{sample}}/called_barcodes_by_SW_method.csv"
    output:
        fq_R1="slim_fastq_{template}/{sample}_R1.fastq.gz",
        fq_R2="slim_fastq_{template}/{sample}_R2.fastq.gz"
    run:
        job_name=f'Car_{wildcards.sample}'
        if cfg_type=='sc10xV3':
            command=f"papermill {QC_dir}/extra-gen_10xv3_fastq.ipynb -k {kernel} DARLIN/{wildcards.template}_{python_sub_dir}/{wildcards.sample}/extra-gen_10xv3_fastq.ipynb -p called_bc_file {input.called_bc_file} -p locus {wildcards.template} -p R1_file {output.fq_R1} -p R2_file {output.fq_R2}"
        elif cfg_type=='scCamellia':
            command=f"papermill {QC_dir}/extra-gen_scCamellia_fastq.ipynb -k {kernel} DARLIN/{wildcards.template}_{python_sub_dir}/{wildcards.sample}/extra-gen_scCamellia_fastq.ipynb -p called_bc_file {input.called_bc_file} -p locus {wildcards.template} -p R1_file {output.fq_R1} -p R2_file {output.fq_R2}"
        if config['sbatch']==0:
...
rule DARLIN:        
    input:
        ...
        fq_R2="slim_fastq_{template}/{sample}_R2.fastq.gz"
    output:
        "DARLIN/{template}_cutoff_override_1/{sample}/Actaul_CARLIN_seq.txt",
        "DARLIN/{template}_cutoff_override_1/{sample}/AlleleAnnotations.txt"
    run:
        from darlin.settings import CARLIN_dir
        print(CARLIN_dir)
        CARLIN_dir=hf.update_CARLIN_dir(CARLIN_dir,wildcards.template)
        print("Updated CARLIN_dir:"+ str(CARLIN_dir))
        output_dir=config['data_dir']+f'/DARLIN/{wildcards.template}_cutoff_override_1'
        input_dir=config['data_dir']+f'/slim_fastq_{wildcards.template}'
        print(input_dir)
        ...
        command_1=f"bash {script_dir}/run_CARLIN.sh {CARLIN_dir} {input_dir} {output_dir} {wildcards.sample} {cfg_type} {wildcards.template} {read_cutoff_UMI_override} {read_cutoff_CB_override} {requested_memory} {0} {CARLIN_max_run_time}"
...
rule join_results:
    input:
        ...
        "DARLIN/{template}_cutoff_override_1/{sample}/AlleleAnnotations.txt"
    output:
        touch(f"DARLIN/{{template}}_{python_sub_dir}/{{sample}}/final.done")
    run:
        called_bc_file = f"DARLIN/{wildcards.template}_{python_sub_dir}/{wildcards.sample}/called_barcodes_by_SW_method.csv"
        carlin_seq_file = f"DARLIN/{wildcards.template}_cutoff_override_1/{wildcards.sample}/Actaul_CARLIN_seq.txt"
        allele_annot_file = f"DARLIN/{wildcards.template}_cutoff_override_1/{wildcards.sample}/AlleleAnnotations.txt"
        final_file = f"DARLIN/{wildcards.template}_{python_sub_dir}/{wildcards.sample}/called_barcodes_by_SW_method_allele.csv"
